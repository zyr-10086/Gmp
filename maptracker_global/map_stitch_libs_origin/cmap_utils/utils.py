...
def draw_polylines(vecs, roi_size, origin):
    results = []
    canvas_size = (100, 100) 
    thickness = 3 
    for line_coords in vecs:
        canvas = np.zeros((canvas_size[1], canvas_size[0]), dtype=np.uint8)
        coords = (line_coords - origin) / roi_size * np.array(canvas_size)
        # coords = coords.numpy()
...
def draw_polygons(vecs, roi_size, origin):
    results = []
    canvas_size = (100, 100) 
    for poly_coords in vecs:
        mask = Image.new("L", size=(canvas_size[0], canvas_size[1]), color=0)
        coords = (poly_coords - origin) / roi_size * np.array(canvas_size)
        # coords = coords.numpy()
...
def draw_instance_masks(vectors, roi_size, origin):
    masks = {}
    canvas_size = (100, 100) 
    canvas = np.zeros((canvas_size[1], canvas_size[0]))
    for label, vecs in vectors.items():
        if label == 0:
            masks[label] = draw_polygons(vecs, roi_size, origin)
        else:
            masks[label] = draw_polylines(vecs, roi_size, origin)
