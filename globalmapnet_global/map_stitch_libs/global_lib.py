...
class MapBuilder:
    def __init__(self, patch_size=(30, 60), root_dir=r'./cache/global_maps', threshold=0.05, cache_patch_traces=True, cross_scene_eval=False):
...
def plot_fig_merged(car_trajectory, x_min, x_max, y_min, y_max, pred_save_path, global_map_elements):
    ...
    for element in global_map_elements:
        ...
        polyline = LineString(element['coords'] )
...
def export_eval_data(pred_global_datas, gt_global_datas, export_path):

    eval_datas = {}
    pred_datas = {'divider': [], 'ped_crossing': [], 'boundary': []}
    gt_datas = {'divider': [], 'ped_crossing': [], 'boundary': []}

    for pred_element in pred_global_datas:
    ...
    for gt_element_by_cate in gt_global_datas:
    ...
    eval_datas['pred'] = pred_datas
    eval_datas['gt'] = gt_datas
    # import ipdb; ipdb.set_trace()
    with open(export_path, 'wb') as f:
        pkl.dump(eval_datas, f)

def export_eval_data_all(all_eval_data, export_path):

    all_eval_data_export = {}


    for scene_name, eval_data in all_eval_data.items():
        pred_global_datas, gt_global_datas = eval_data
        # import ipdb; ipdb.set_trace()
        ...
        all_eval_data_export[scene_name] = eval_datas

    # import ipdb; ipdb.set_trace()
    with open(export_path, 'wb') as f:
        pkl.dump(all_eval_data_export, f)