...
def preprocess_frame(frame):
    ...
    dim = gt_box[0].shape[1]
    if dim == 3:
        gt_box = np.array([gb[...,:2] for gb in gt_box])
        pred_box =np.array([pb[...,:2] for pb in pred_box])
...
def transform_coords(coords, position, yaw):
    ...
    if dim == 2:
        ...
        return np.dot(coords[..., :2], rotation_matrix.T) + position
...
def update_global_map(global_map, message, type = "pred"):
    ...
    for p_box, p_label in zip(box, label):
        # import ipdb;ipdb.set_trace()
        global_coords = transform_coords(p_box, message['position'], message['yaw'] - np.pi/2)
        # global_coords = p_box
        # import ipdb;ipdb.set_trace()
...
def update_global_map_all(global_map, gt_elements):
    label2id = {
        'divider': 0,
        'ped_crossing': 1,
        'boundary': 2
...
def polygon_merge_check(poly1, poly2, T):
    """
    Function that checks if the two polylines should be merged or not. 
    Inputs: 
    polyline1, polyline2 ->
...
def merge_polys_v3(poly1, poly2, category,pre = True, denoise=True):
    """
    ...
    if category == 0:
        ...
        denoise_thr = 1.5
        # denoise_thr = 1.0 0.5 0.4187
        # denoise_thr = 1.5 0.5 0.4224
        # denoise_thr = 1.5 0.8 0.4196
        # denoise_thr = 0.8 0.5 0.4130
        insert_thr = 0.5
        # insert_thr = 0.8
    else:
        # denoise_thr = 1.5
    ...
    if denoise:
        ...
        for i in range(len(polyM)):
            ...
            dist = math.dist(p,p4)
            if not(all(p4==p2) or all(p4==p3)) and dist < denoise_thr:
                polyM[i] = (p+p4)/2
    ...
    # if category == 1:
    #     ped = shapely.geometry.Polygon([[p[0], p[1]] for p in polyM_resample]).convex_hull
    #     polyM_resample = np.array(ped.exterior.coords)
...
def merge_recursive(polys, result, category, pre = True, denoise = True):
    ...
    # 递归地合并两半的多边形
    left_merged, _ , _ = merge_recursive(left_half, result, category, pre=pre, denoise=denoise)
    right_merged, _ , _ = merge_recursive(right_half, result, category, pre=pre, denoise=denoise)
    result[0].append(left_merged)
    result[1].append(right_merged)
 
    # 合并两个递归调用的结果
    return merge_polys_v3(left_merged, right_merged, category, pre = pre, denoise=denoise), left_merged, right_merged
...
def refine_token_v2(global_map, proximity_th, save = False, name = 'data.pkl', pre = True, denoise = True):
    """
    ...
    for category in range(0,3):
        ...
        for idx1, poly1 in enumerate(flattened_list):
            ...
            for idx2, poly2 in enumerate(poly2_list):
                if category != 1:
                    ...
                    # check = polygon_merge_check(np.array(poly1), np.array(poly2), proximity_th)
                    d1, d2, check = polyline_merge_check(np.array(poly1), np.array(poly2), proximity_th)
                # d1, d2, check = hausdorff_distance(np.array(poly1), np.array(poly2), proximity_th)
        ...
        for idxs2merge in C:
            ...
            merged_poly, left, right = merge_recursive(instance_set, result, category, pre = pre, denoise = denoise)
            # import ipdb;ipdb.set_trace()
            # temp_1 = [[],[],[]]
...
def refine_token_v3(global_map, proximity_th, save = False, name = 'data.pkl', pre = True, denoise = True):
...
def plot_fig_merged(car_trajectory, x_min, x_max, y_min, y_max, pred_save_path, global_map):
    ...
    for label in range(len(global_map)):
        ...
        vecs = [vec for frame in global_map[label] for vec in frame]
        # colors = [np.random.rand(3,) for _ in range(len(vecs))] 
